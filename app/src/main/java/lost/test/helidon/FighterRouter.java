...
public class FighterRouter implements HttpService {
    ...
    private void findAll(ServerRequest req,
    ...
    private void findByName(ServerRequest req,
                            ServerResponse res) throws Exception {
    ...
    private void add(ServerRequest req,
                     ServerResponse res) throws Exception {

        var fighterCreate = req.content().as(FighterCreate.class);
        var fighterInsert = FighterBuilder.builder().name(fighterCreate.name()).addSkill(fighterCreate.skill()).createdAt(now(UTC)).build();
    ...
    private void edit(ServerRequest req,
                      ServerResponse res) throws Exception {
        ...
        try (var tx = db.beginTransaction()) {
            var found = db.find(Fighter.class).where().eq("name", fighterEdit.name()).findOneOrEmpty().orElseThrow();
            fighterUpdate = FighterBuilder.builder(found).skill(fighterEdit.skill()).updatedAt(now(UTC)).build();
    ...
    private void delete(ServerRequest req,
                        ServerResponse res) throws Exception {
